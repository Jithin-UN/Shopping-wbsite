...
export default function Admin({ products }: AdminProps) {
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    try {
      ...
      // No re-throw here so we can show a user friendly alert if needed, 
      // but the handleFirestoreError helper would log it anyway.
...
-------------------------------------------
  ...
  React.useEffect(() => {
    if (categories.length > 0 && !formData.category && !editingProduct) {
        ...
        <div>
          ...
          <div className="flex items-center space-x-6 mt-4">
            ...
            <button 
              ...
              className={`pb-2 text-sm font-bold uppercase tracking-widest transition-all border-b-2 ${activeTab === 'categories' ? 'text-indigo-600 border-indigo-600' : 'text-gray-400 border-transparent hover:text-gray-600'}`}
            >
              Categories
            </button>
          </div>
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-2">
                    <label className="block text-sm font-bold text-gray-700 mb-4 uppercase tracking-wider">Available Sizes</label>
      ...
      {activeTab === 'settings' ? (
        ...
        >
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-8">
              {isAddingCategory && (
                <form onSubmit={handleAddCategory} className="mb-8 p-6 bg-gray-50 rounded-2xl border border-gray-100 flex gap-4">
                  <input
                    type="text"
                    required
                    autoFocus
                    ...
                    className="flex-grow px-4 py-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    placeholder="Category name (e.g. Traditional)"
                  />
                  <button
                    type="submit"
                    disabled={loading}
              ...
              {categoriesLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">No categories found. Add your first category to start organizing products.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {categories.map((cat) => (
                    <div key={cat.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all">
